...
void drawAxis()
{
  ...
  for (int x = 0; x < 128; x += 10) {
    for (int y = 0; y <4 and x < 128; y++, x += 8){
...
void drawFrameTime(unsigned long us, unsigned long  U )
{
  if (U >  1000) display.setCursor(0 * (CHARWIDTH - 1), 7 * CHARHEIGHT - 2); // almost at bottom, approximately centered
  if (U <= 1000) display.setCursor(0 * (CHARWIDTH - 1), 6 * CHARHEIGHT - 7); // almost at bottom, approximately centered
  displayln("%ld us", us);
}

// Shows the max. value on screen
void drawPeakValue(unsigned long  U)
{
  if (U >  1000) display.setCursor(11 * (CHARWIDTH - 1), 7 * CHARHEIGHT - 2); // almost at bottom, approximately centered
  if (U <= 1000) display.setCursor(11 * (CHARWIDTH - 1), 6 * CHARHEIGHT - 7); // almost at bottom, approximately centered
  displayln("peak:%4ld mV", U);
}
...
void drawFull()
{
  ...
  for (int x = 0; x < 22; x++) {
    for (int y = 0; y < 8; y++) {
      display.drawChar(x * CHARWIDTH, y * CHARHEIGHT, c, BLACK, INVERSE, 1);
    }
  }
}
...
void loop() {
  ...
  if ( (++count) % sampleRate == 0 ){
    ...
    values[pos++] = value >> 4; // shifting right by 4 efficiently maps 0-1023 range to 0-63
  }
  ...
  if ( pos >= NUMVALUES ) {
    ...
    if ( !digitalRead(SCREEN_UPDATE_ENABLE_PIN) ) {
      ...
      drawAxis();
      drawValues();
      drawFrameTime(totalSampleTime,U);
      drawPeakValue(U);
